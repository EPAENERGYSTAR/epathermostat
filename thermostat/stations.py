...
def _get_both_year_station(lat, lon, max_dist_km=_MAX_STATION_DISTANCE_KM):
    ...
    with both 2025 and 2026 hourly data present in the local eeweather cache,
    within *max_dist_km* kilometres.
    ...
    for usaf_id, row in station_ranking.iterrows():
        ...
        if (store.key_exists('isd-hourly-{}-2025'.format(usaf)) and
                store.key_exists('isd-hourly-{}-2026'.format(usaf))):
            return usaf
    return None
...
def get_closest_station_by_zipcode(zipcode):
    ...
    has both 2025 and 2026 hourly data in the local cache, within 500 km of
    the ZCTA centroid. Falls back to the static JSON map when eeweather does
    not recognise the ZCTA or no qualifying station is found in range.
