...
def test_zcta_coverage_rate(pytestconfig, tmp_path):
    ...
    # zipcode_usaf_station.json is generated by scripts/build_station_data.py build-lookup
    # using the same _get_both_year_station filter, so its values are already the
    # nearest station with cache data for the current [year-1, year] window.
    # ZCTAs absent from the map (island territories + remote Alaska) have no
    # both-year station in range — a documented coverage gap.
