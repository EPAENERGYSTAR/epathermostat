...
def test_get_heating_season_baseline_setpoint_null_temperature_in(thermostat_template):
    ...
    heating_season = Season("FAKE", season_selector, season_selector, None, None)

    sp = thermostat_template.get_heating_season_baseline_setpoint(
            heating_season, source='heating_setpoint')
    assert pd.isnull(sp)

    sp = thermostat_template.get_heating_season_baseline_setpoint(
            heating_season, source='temperature_in')
...
def test_get_cooling_season_baseline_setpoint_null_temperature_in(thermostat_template):
    ...
    season_selector = pd.Series([True, True, True], index=index)
    cooling_season = Season("FAKE", season_selector, season_selector, None, None)

    sp = thermostat_template.get_cooling_season_baseline_setpoint(
            cooling_season, source='cooling_setpoint')
    assert pd.isnull(sp)

    sp = thermostat_template.get_cooling_season_baseline_setpoint(
            cooling_season, source='temperature_in')
...
def test_get_heating_season_baseline_setpoint_null_heating_setpoint(thermostat_template):
...
def test_get_cooling_season_baseline_setpoint_null_cooling_setpoint(thermostat_template):
