...
from .fixtures.single_stage import (
    ...
    thermostat_type_1_minutes_over_60_cooling,
    thermostat_type_1_minutes_over_60_heating,
    thermostat_type_1_minutes_over_60_aux,
    thermostat_type_1_minutes_over_60_emergency,
)
...
def test_import_csv(thermostat_type_1):
    ...
    def assert_is_dataframe_with_shape(dataframe, shape):
    ...
    assert_is_dataframe_with_shape(
        thermostat_type_1.cool_runtime_hourly, (35064,2))
    assert_is_dataframe_with_shape(
        thermostat_type_1.heat_runtime_hourly, (35064,2))

    assert_is_dataframe_with_shape(
        thermostat_type_1.auxiliary_heat_runtime, (35064,2))
    assert_is_dataframe_with_shape(
        thermostat_type_1.emergency_heat_runtime, (35064,2))
...
def test_utc_offset(thermostat_type_1_utc, thermostat_type_1_utc_bad):
    ...
    assert isinstance(thermostat_type_1_utc.cool_runtime_hourly, pd.DataFrame)
    assert thermostat_type_1_utc_bad is None


def test_too_many_minutes(thermostat_type_1_too_many_minutes):
    # All of these thermostats should still import
    assert len(thermostat_type_1_too_many_minutes) == 4
...
def test_missing_runtime_hours(thermostat_missing_hours):
    ...
    assert len(thermostat_missing_hours.heat_runtime_hourly.loc[[
               '2011-05-08 18:00:00']].dropna()) == 0
    assert len(thermostat_missing_hours.heat_runtime_daily.loc[[
               '2011-05-08']].dropna()) == 1
    assert len(thermostat_missing_hours.heat_runtime_hourly.loc[[
               '2011-05-09 7:00:00']].dropna()) == 0
    assert len(thermostat_missing_hours.heat_runtime_hourly.loc[[
               '2011-05-09 8:00:00']].dropna()) == 0
    assert len(thermostat_missing_hours.heat_runtime_daily.loc[[
               '2011-05-09']].dropna()) == 1
    assert len(thermostat_missing_hours.heat_runtime_daily.loc[[
               '2011-01-01']].dropna()) == 0
...
def test_missing_temperature_over_18_days(thermostat_missing_over_18_days_temperature):
    ...
    assert len(
        thermostat_missing_over_18_days_temperature.enough_temp_in[thermostat_missing_over_18_days_temperature.enough_temp_in]) < 347


def test_missing_temperature_hours(thermostat_missing_temperature):
    assert len(thermostat_missing_temperature.temperature_in.loc[[
               '2011-06-02 6:00:00']].dropna()) == 0
    assert len(thermostat_missing_temperature.temperature_in.loc[[
               '2011-06-02 7:00:00']].dropna()) == 0
    assert len(thermostat_missing_temperature.temperature_in.loc[[
               '2011-06-02 8:00:00']].dropna()) == 0
    assert len(thermostat_missing_temperature.temperature_in.loc[[
               '2012-06-02 18:00:00']].dropna()) == 1
    assert len(thermostat_missing_temperature.temperature_in.loc[[
               '2011-06-28 18:00:00']].dropna()) == 1
    assert len(thermostat_missing_temperature.temperature_in.loc[[
               '2012-06-29 18:00:00']].dropna()) == 1
    assert len(thermostat_missing_temperature.temperature_in.loc[[
               '2014-12-15 20:00:00']].dropna()) == 1
    assert len(thermostat_missing_temperature.temperature_in.loc[[
               '2014-12-31 20:00:00']].dropna()) == 1
    ...
    assert len(thermostat_missing_temperature.temperature_out.loc[[
               '2011-06-26 18:00:00']].dropna()) == 0
    assert len(thermostat_missing_temperature.temperature_out.loc[[
               '2012-06-26 18:00:00']].dropna()) == 1
...
def test_thermostat_type_1_minutes_over_60_aux(thermostat_type_1_minutes_over_60_aux):
    assert len(thermostat_type_1_minutes_over_60_aux.auxiliary_heat_runtime.loc[[
               '2011-01-01']].dropna()) == 0
    assert len(thermostat_type_1_minutes_over_60_aux.auxiliary_heat_runtime.loc[[
               '2011-01-02']].dropna()) == 1


def test_thermostat_type_1_minutes_over_60_emergency(thermostat_type_1_minutes_over_60_emergency):
    assert len(thermostat_type_1_minutes_over_60_emergency.emergency_heat_runtime.loc[[
               '2011-01-01']].dropna()) == 0
    assert len(thermostat_type_1_minutes_over_60_emergency.emergency_heat_runtime.loc[[
               '2011-01-02']].dropna()) == 1


def test_thermostat_type_1_minutes_over_60_cooling(thermostat_type_1_minutes_over_60_cooling):
    assert len(thermostat_type_1_minutes_over_60_cooling.cool_runtime_daily.loc[[
               '2011-01-01']].dropna()) == 0
    assert len(thermostat_type_1_minutes_over_60_cooling.cool_runtime_daily.loc[[
               '2011-01-02']].dropna()) == 0
    assert len(thermostat_type_1_minutes_over_60_cooling.cool_runtime_daily.loc[[
...
def test_thermostat_type_1_minutes_over_60_heating(thermostat_type_1_minutes_over_60_heating):
    assert len(thermostat_type_1_minutes_over_60_heating.heat_runtime_daily.loc[[
               '2011-01-01']].dropna()) == 0
    assert len(thermostat_type_1_minutes_over_60_heating.heat_runtime_daily.loc[[
               '2011-01-02']].dropna()) == 0
    assert len(thermostat_type_1_minutes_over_60_heating.heat_runtime_daily.loc[[
